# Sprint Planning Meeting

## Sprint Goal:

이용자는 아이디, 패스워드를 통해 회원 가입을 한 후 로그인하여 현재 카우버거 매장에서 가능한 메뉴를 카테고리 별로 확인할 수 있다. 또한 특정 메뉴를 클릭하여 해당 메뉴의 가격과 알레르기 정보를 알 수 있다.

## API

[/api/signup](https://clear-magic-8ab.notion.site/auth-signup-89725220be684e80a1143f09a44dc266)  
[/api/signin](https://clear-magic-8ab.notion.site/auth-signin-0915bcfc3eaa49eaac27ee675ff41484)

### sub groub1

문제 해결 : UC -1 : 회원가입, UC -2 : 로그인

참여 : 김효진(Front-End : UI마크업, 로그인 및 회원가입 api 연동), 정설희(Back-End : 로그인 및 회원가입 api 설계)

기간 : 12월 3일 - 12월 8일

<img src="../../project/frontend/cau-burger-online-order-system/public/login_signup.gif" width="70%">

## UC-1 회원가입

what:

- 서비스를 이용하기 위해, 이용자는 서비스에 회원 가입한다. 회원 가입 정보는 사용자 이름, 아이디, 패스워드, 핸드폰 번호를 포함한다.

how:

- 회원가입 페이지를 구성하고 이용자가 입력한 정보를 받아서 데이터베이스에 저장한다.
- 회원가입시, 중복된 정복된 정보 검사

req - 1 : 서비스를 이용하기 위해, 이용자는 서비스에 회원 가입한다. 회원 가입 정보는 사용자 이름, 아이디, 패스워드, 핸드폰 번호를 포함한다.

| Use Case UC-1        | 회원가입                                                                                 |
| -------------------- | ---------------------------------------------------------------------------------------- |
| Related Requirement  | REQ-1                                                                                    |
| Initialing Actor     | 이용자                                                                                   |
| Actor's Goal         | 이용자는 서비스에 회원가입을 한다. 정보 입력시 이용자 이름, 아이디, 패스워드를 포함한다. |
| Participating Actors | 유저 정보 DB                                                                             |
| Preconditions        | 카우버거 온라인 주문 시스템 홈페이지에 접속하여 회원가입 페이지로 이동한다.              |
| Post Conditions      | 카우버거 온라인 주문 시스템에 유저의 계정이 생성된다.                                    |
| Flow                 |
| →1.                  | 이용자는 이름, 아이디, 패스워드를 입력하여 계정 생성을 요청한다.                         |
| ←2.                  | 입력된 아이디가 로그인 정보 DB에 일치하는 정보가 없으면 계정을 생성한다.                 |
| →3.                  | 이용자는 계정 생성을 완료한다.                                                           |

![image](https://user-images.githubusercontent.com/71129059/144805440-b74f0362-499c-4621-9f08-d5ba53b9f349.png)

## UC-2 로그인

what:

- 이용자는 아이디와 비밀번호를 입력해 로그인한다.
- 서비스 관리를 위해, 관리자는 관리자 계정으로 서비스에 로그인 할 수 있다.

how:

- 로그인 페이지를 구성하고 이용자가 입력한 회원 정보와 데이터베이스의 정보와 일치하는 지 확인한다.
- 관리자 계정은 따로 미리 생성

req - 2 : 이용자는 아이디와 비밀번호를 입력해 로그인한다.

req - 16 : 서비스 관리를 위해, 관리자는 관리자 계정으로 서비스에 로그인 할 수 있다

| Use Case UC-2        | 로그인                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------- |
| Related Requirement  | REQ-2, REQ-16                                                                                |
| Initialing Actor     | 이용자, 관리자                                                                               |
| Actor's Goal         | 이용자 및 관리자는 회원가입할 때 입력한 정보로 홈페이지에 로그인한다.                        |
| Participating Actors | 로그인 정보 DB                                                                               |
| Preconditions        | 카우버거 온라인 주문 시스템 홈페이지에 접속하여 로그인 페이지로 이동한다.                    |
| Post Conditions      | 카우버거 온라인 주문 시스템 홈페이지를 이용할수 있다.                                        |
| Flow                 |
| →1.                  | 이용자는 아이디, 패스워드를 입력하여 로그인한다.                                             |
| ←2.                  | 입력받은 정보가 로그인 정보 DB에 일치하는지 여부를 전송한다.                                 |
| →3.                  | 정보가 일치하면 로그인에 성공하고, 일치하지 않으면 로그인에 실패한다. 실패하면 1로 돌아간다. |

![image](https://user-images.githubusercontent.com/71129059/144805573-ce498378-13fe-42a7-b20d-5069854538a8.png)

### sub groub2

진행중 git branch : [subgroup1](https://github.com/hy57in/2021-Industry-Hands-On-Project/tree/subgroup-1/project/frontend/cau-burger-online-order-system)

문제 해결 : UC - 3 : 메뉴조회, UC -4 : 메뉴정보확인

참여 : 조현민, 채희선, 김소현

기간 : 12월 3일 - 12월 8일

| 조현민 | Main 페이지 (UC-3) (main)             |
| ------ | ------------------------------------- |
| 채희선 | 데이터 생성, Main 페이지 (UC-3) (sub) |
| 김소현 | 상세 페이지 (UC-4) (main)             |

<img src="../../project/frontend/cau-burger-online-order-system/public/menu.gif" width="70%">

## UC-3 메뉴 조회

what:

- 현재 등록된 메뉴를 카테고리 별로 확인하고 특정 메뉴를 클릭하여 상세 정보로 이동할 수 있다.

how:

- 홈화면 페이지를 구성하고 메뉴의 데이터들을 불러와서 이용자가 한 화면에서 볼 수 있게 한다.
- 메뉴 데이터는 카테고리로 구분하고, 각각 메뉴 이름, 가격, 메뉴 이미지, 좋아요 정보를 가지고 있다.

req -3 : 이용자는 어떤 메뉴가 있는지 모르므로 메뉴 조회를 통해 메뉴를 알 수 있고 이를 통해 버거,사이드,디저트 종류를 선택할 수 있다.

req - 13: 인기 있는 메뉴를 확인하기 위해, 이용자는 좋아요 순서로 메뉴를 정렬할 수 있다.

| Use Case UC-3        | 메뉴 조회                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------------ |
| Related Requirement  | REQ-3, REQ-13                                                                              |
| Initialing Actor     | 이용자                                                                                     |
| Actor's Goal         | 현재 등록된 메뉴를 카테고리 별로 확인하고 특정 메뉴를 클릭하여 상세 정보로 이동할 수 있다. |
| Participating Actors | 메뉴 정보 DB                                                                               |
| Preconditions        | 이미 등록된 메뉴가 데이터 베이스에 저장되어 있다.                                          |
| Post Conditions      | 이용자는 원하는 카테고리를 설정할 수 있고 원하는 메뉴를 클릭하여 상세정보로 이동한다.      |
| Flow                 |
| →1.                  | 이용자가 메뉴확인 페이지에 접속한다.                                                       |
| ←2.                  | 이용자가 버거, 사이드, 디저트 중 원하는 카테고리를 선택한다.                               |
| →3.                  | 이용자가 원하는 메뉴를 선택하면 상세정보 페이지로 이동한다.                                |

## UC-4 메뉴 정보 확인

what:

- 판매중인 메뉴의 가격 및 세부정보를 확인할 수 있게한다.
- 이용자가 메뉴판을 가독성 있게 읽고 주문할 수 있도록, 카테고리에서 버거, 사이드, 디저트로 구분하고 메뉴를 선택하여 상세페이지를 통해 장바구니에 담을 수 있게 한다.
- 이용자가 메뉴의 세부 정보를 확인하고 주문할 수 있도록, 상세 페이지를 통해 메뉴를 읽고 메뉴 개수를 선택할 수 있게 한다.

how:

- 메뉴 세부정보 페이지를 구성하고 홈화면 페이지에서 특정 메뉴를 선택하면 해당 메뉴의 세부정보를 데이터에서 불러와 이용자가 볼 수 있게 한다.
- 수량은 1개 이상 10개 이하만 선택할 수 있게 한다.
- 세부 페이지에서 이용자가 주문을 원하지 않을 경우 홈화면을 통해 돌아갈 수 있다.

req - 5 : 메뉴에 대한 정보를 알기 위해, 이용자가 특정 메뉴를 선택하면 해당 메뉴에 대한 가격 및 세부정보(구성 정보)를 제공한다.

req - 8: 이용자가 메뉴판을 가독성 있게 읽고 주문할 수 있도록, 카테고리에서 버거, 사이드, 디저트로 구분하고 메뉴를 선택하여 상세페이지를 통해 장바구니에 담을 수 있게 한다.

req - 9: 이용자가 메뉴의 세부 정보를 확인하고 주문할 수 있도록, 상세 페이지를 통해 메뉴를 읽고 메뉴 개수를 선택할 수 있게 한다.

| Use Case UC-4        | 메뉴 정보 확인                                                                                                 |
| -------------------- | -------------------------------------------------------------------------------------------------------------- |
| Related Requirement  | REQ-5, REQ-8, REQ-9                                                                                            |
| Initialing Actor     | 이용자                                                                                                         |
| Actor's Goal         | 판매중인 메뉴의 가격 및 세부정보를 확인하고, 구매 희망 수량만큼 장바구니에 추가 가능하다.                      |
| Participating Actors | 메뉴 정보 DataBase                                                                                             |
| Preconditions        | 사용자가 전체 메뉴 조회 페이지에서 특정 메뉴를 선택해야하고, 해당 메뉴의 상세정보가 DB에 저장되어 있어야 한다. |
| Post Conditions      | 사용자가 해당 메뉴를 원하는 수량만큼 장바구니에 담거나, 뒤로가기하여 전체 메뉴 페이지로 돌아갈 수 있다.        |
| →1.                  | 사용자가 전체 메뉴 조회 페이지에서 특정 메뉴를 선택한다                                                        |
| ←2.                  | DB에서 해당 메뉴에 대한 가격 및 상세정보를 불러온다                                                            |
| →3.                  | 사용자가 가격 및 상세정보 확인 후, 해당 메뉴의 구매 희망 수량을 입력하고 주문버튼을 누른다                     |
| ←4.                  | 입력 수량만큼 사용자의 장바구니에 추가시킨 후 전체 메뉴 조회 페이지로 돌아간다.                                |

![image](https://user-images.githubusercontent.com/80442377/144801566-a2fef197-90c9-466b-acfc-e18c89c528b2.png)
